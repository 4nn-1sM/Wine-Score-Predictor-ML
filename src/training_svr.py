...
Selectkbest_param = {
    ...
    'feature_selection__k': np.arange(1,15,1),  
    "regressor__C": np.arange(0.1, 2.0, 0.1),  # Regularización entre 0.1 y 2.0
    "regressor__kernel": ["linear", "rbf", "poly", "sigmoid"],  # Diferentes kernels para probar
    "regressor__gamma": np.arange(0.01, 1.0, 0.1),  # Rango para gamma
    "regressor__epsilon": np.arange(0.1, 0.5, 0.05) 
}
Cluster_param = {
    'scaler': [StandardScaler(), MinMaxScaler(), RobustScaler(), None],
    'clustering__n_clusters': np.arange(1,15,1),
...
PCA_param = {
    'scaler': [StandardScaler(), MinMaxScaler(), RobustScaler(), None],
    'pca__n_components': np.arange(1,25,1),  
    "regressor__C": np.arange(0.1, 2.0, 0.1),  # Regularización entre 0.1 y 2.0
    "regressor__kernel": ["linear", "rbf", "poly", "sigmoid"],  # Diferentes kernels para probar
    "regressor__gamma": np.arange(0.01, 1.0, 0.1),  # Rango para gamma
    "regressor__epsilon": np.arange(0.1, 0.5, 0.05) 
