...
def entrenar_y_guardar_modelos(pipes, params, X_train, y_train, X_test, y_test, modelos, decision_base=1):
    ...
    for pipe, param, modelo in zip(pipes, params, modelos):
        ...
        resultado["train_results"].to_csv(f"../models/LinearRegression/train_results_{modelo}{decision}.csv", index=False)
        print(f"Resultados train del modelo {modelo} guardados en CSV.")

        resultado["test_results"].to_csv(f"../models/LinearRegression/test_results_{modelo}{decision}.csv", index=False)
        print(f"Resultados test del modelo {modelo} guardados en CSV.")

        # Guardar el mejor modelo en PICKLE
        pickle.dump(resultado["best_estimator"], open(f"../models/LinearRegression/best_estimator_{modelo}{decision}.pkl", "wb"))
        print(f"Best estimator del modelo {modelo} guardado en PICKLE.")

        # Guardar los mejores parámetros en YAML
        yaml.dump(resultado["best_params"], open(f"../models/LinearRegression/best_params_{modelo}{decision}.yaml", "w"), default_flow_style=False, allow_unicode=True)
        print(f"Best params del modelo {modelo} guardados en YAML.")

        # Guardar el mejor score en CSV
        pd.DataFrame({"best_score": [resultado["best_score"]]}, index=[f'{modelo}{decision}']).to_csv(f"../models/LinearRegression/best_score_{modelo}{decision}.csv")
