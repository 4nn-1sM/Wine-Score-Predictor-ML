...
Selectkbest_pipe = Pipeline(steps=[
    ...
    ("regressor", RandomForestRegressor(n_estimators = 100, random_state=63))
])
...
Selectkbest_param = {
    ...
    'regressor__n_estimators': np.arange(1,200,20), 
    'regressor__max_depth': [None] + list(np.arange(1,20)),
...
PCA_param = {
    ...
    'regressor__n_estimators': np.arange(1,200,20),  # Número de árboles
    'regressor__max_depth': [None] + list(np.arange(1,20)),
    "regressor__max_features": ["sqrt","log2", None] 
}
...
def entrenar_y_guardar_modelos(pipes, params, X_train, y_train, X_test, y_test, modelos, decision_base=1):
    ...
    for pipe, param, modelo in zip(pipes, params, modelos):
        ...
        resultado["train_results"].to_csv(f"../models/RandomForestRegressor/train_results_{modelo}{decision}.csv", index=False)
        print(f"Resultados train del modelo {modelo} guardados en CSV.")

        resultado["test_results"].to_csv(f"../models/RandomForestRegressor/test_results_{modelo}{decision}.csv", index=False)
        print(f"Resultados test del modelo {modelo} guardados en CSV.")

        # Guardar el mejor modelo en PICKLE
        pickle.dump(resultado["best_estimator"], open(f"../models/RandomForestRegressor/best_estimator_{modelo}{decision}.pkl", "wb"))
        print(f"Best estimator del modelo {modelo} guardado en PICKLE.")

        # Guardar los mejores parámetros en YAML
        yaml.dump(resultado["best_params"], open(f"../models/RandomForestRegressor/best_params_{modelo}{decision}.yaml", "w"), default_flow_style=False, allow_unicode=True)
        print(f"Best params del modelo {modelo} guardados en YAML.")

        # Guardar el mejor score en CSV
        pd.DataFrame({"best_score": [resultado["best_score"]]}, index=[f'{modelo}{decision}']).to_csv(f"../models/RandomForestRegressor/best_score_{modelo}{decision}.csv")
